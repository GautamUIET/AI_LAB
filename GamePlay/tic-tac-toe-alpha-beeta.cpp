...
int rate(const vector<int>& b) {
    ...
    if (checkWin(b) == aiSymbol) {      //Check Winning state.
        return 1000;
    } else if (checkWin(b) == userSymbol) {
        return -1000;
    }

    for (int i = 0; i < 3; i++) {       //Check Possible Win.
    ...
    // Strategic positions
    if (b[4] == aiSymbol) score += 15; 
    if (b[4] == userSymbol) score -= 15; 

    // Corner control
    int corners[] = {b[0], b[2], b[6], b[8]};
    for (int corner : corners) {
        if (corner == aiSymbol) score += 3; 
        if (corner == userSymbol) score -= 3; 
    }

    // Edge control
    int edges[] = {b[1], b[3], b[5], b[7]};
    for (int edge : edges) {
        if (edge == aiSymbol) score += 1; 
        if (edge == userSymbol) score -= 1; 
    }
...
int minimax(vector<int>& board, int depth, bool isMaximizing, int maxDepth, int alpha, int beta) {
    total++;
    ...
    if (isMaximizing) {
        ...
        for (int move : move_gen(board)) {
            ...
            bestScore = max(bestScore, minimax(board, depth + 1, false, maxDepth, alpha, beta));
        ...
        for (int move : move_gen(board)) {
            ...
            bestScore = min(bestScore, minimax(board, depth + 1, true, maxDepth, alpha, beta));
            board[move] = 2; // Undo the move
...
void computermove(int maxDepth) {
    ...
    for (int move : move_gen(board)) {
        board[move] = aiSymbol; // AI's move
        int score = minimax(board, 0, false, maxDepth, alpha, beta);
        board[move] = 2; // Undo the move
        if (score > bestScore) {
            bestScore = score;
            bestMove = move;
